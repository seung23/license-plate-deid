...
본 연구는 차량 이미지 내 번호판 정보를 단순히 가리는 방식(Blurring, Mosaic)에서 벗어나, 이미지의 시각적 품질과 스타일을 유지하면서 번호판 정보를 자연스럽게 비식별화하는 엔드투엔드(End-to-End) 파이프라인을 제안합니다. 

---

## ⚠️ 실행 환경 안내 (Disclaimer)
특정 GPU 서버 환경에 최적화된 라이브러리 종속성으로 인해, 일반적인 로컬 환경에서의 단순 빌드 및 실행이 어렵습니다.

---

## 🏗️ 시스템 파이프라인 (System Pipeline)
본 시스템은 세 가지 핵심 모델이 유기적으로 연결된 다단계 파이프라인으로 구성됩니다. 
